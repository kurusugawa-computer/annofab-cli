...
### Python
* dictから値を取得する際、必須なキーならばブラケット記法を使う。キーが必須かどうか分からない場合は、必須とみなし、ブラケット記法を使う（例: `task["key"]`）。
...
5. ドキュメント（`docs/command_reference/`以下）を修正した場合は、`make docs`を実行してビルドエラーがないことを確認する。
6. コードの修正が完了したら、対象コマンドのドキュメント（`docs/command_reference/`以下）を修正すべきかどうか検討する。ドキュメントの修正が必要な場合は、忘れずに修正する。
7. 必要ならばテストコードも記載する。
