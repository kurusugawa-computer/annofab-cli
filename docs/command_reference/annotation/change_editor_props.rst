...
チェッカーロールで変更した場合、変更後は未着手状態になります。

デフォルトでは休憩中状態のタスクに含まれるアノテーションも変更できません。休憩中状態のタスクに含まれるアノテーションも変更する場合は、 ``--include_break_task`` を指定してください。

オーナーロールでは、タスクの担当者や状態を変更せずに ``editor_props`` を変更できます。
チェッカーロールで自身が担当者ではないタスクの ``editor_props`` を変更するには、 ``--change_operator_to_me`` を指定してください。担当者を一時的に自分自身に変更して変更を実行します。オーナーロールで指定しても効果はありません。
