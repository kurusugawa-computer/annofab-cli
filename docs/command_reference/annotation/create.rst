...
休憩中状態のタスクは既定ではスキップします。作成対象に含めるには ``--include_break_task`` を指定してください。
...
チェッカーロールで自身が担当者ではないタスクに作成する場合は、``--change_operator_to_me`` を指定してください。担当者を一時的に自分自身に変更し、作成後に元へ戻します。
