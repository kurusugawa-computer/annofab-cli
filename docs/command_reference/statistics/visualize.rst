...
``inspection_reached`` を指定すると、検査フェーズに到達したタスクを「生産したタスク（作業が完了したタスク）」とみなして、検査フェーズと受入フェーズのタスクを集計対象にします。
受入フェーズに到達したタスクが少ないため、暫定的に検査フェーズに到達したタスクから生産性を算出するときなどに有用なオプションです。
...
``annotation_started`` を指定すると、教師付フェーズが着手されたタスクを「生産したタスク（作業が完了したタスク）」とみなして、教師付フェーズが着手されたタスクを集計対象にします。
教師付フェーズのタスクも生産性の評価対象タスクとして算出したい場合に有用なオプションです。たとえば、以下のようなケースです。

* 受入フェーズ完了状態になってからアノテーションルールが変わったため、大量のタスクを教師付フェーズに差し戻した。これらのタスクは一旦完成しているので、生産性の評価対象にしたい。
* アノテーションルールで不明な点があるため、教師付フェーズ保留状態のタスクが大量にある。これらのタスクは未完成だが約90%は完了しているため、生産性の評価対象タスクに加えたい。

``acceptance_reached`` や ``inspection_reached`` とは異なり、``annotation_started`` では後段の検査/受入フェーズの作業時間も0にしません。
``acceptance_reached`` や ``inspection_reached`` は、アノテーション受注側と発注側で作業フェーズが分かれている運用を想定していますが、``annotation_started`` はそうではないためです。
