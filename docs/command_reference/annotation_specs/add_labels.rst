...
.. code-block::
    ...
    ,pedestrian,,segmentation_v2,#123456,"{""display_name"": {""_type"": ""DisplayName"", ""text"": ""歩行者""}}"
...
.. note::
    ...
    ``--label_json`` / ``--label_csv`` では ``field_values`` も指定できます。 ``--label_json`` では ``field_values`` にJSONオブジェクトを指定してください。
    ``--label_csv`` では ``field_values`` 列にJSONオブジェクト文字列を指定してください。
