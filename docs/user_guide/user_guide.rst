...
``completion`` コマンドで補完スクリプトを生成すると、コマンド名、サブコマンド、オプション、および選択肢を補完できます。補完時にはannofabcliを起動しないため、Web APIへアクセスせず、入力のたびに待たされることもありません。コマンドの構文は :doc:`コマンドリファレンス <../command_reference/completion>` を参照してください。
