...
import annofabapi
from annofabapi.models import Task, AdditionalDataDefinition
...
class ListAnnotationCount(AbstractCommandLineInterface):
    ...
    @staticmethod
    def _modify_attribute_of_query(attribute_query: Dict[str, Any], additional_data_definition: AdditionalDataDefinition) -> Dict[str, Any]:
        if 'choice_name_en' in attribute_query:
            choice_info = more_itertools.first_true(additional_data_definition['choices'],
                                                                   pred=lambda
                                                                       e: AnnofabApiFacade.get_choice_name_en(e) == attribute_query[
                                                                              'choice_name_en'])
            ...
            if choice_info is not None:
                ...
                logger.warning(
                    f"choice_name_en = {attribute_query['choice_name_en']} の選択肢は存在しませんでした。")

        return attribute_query


    def _modify_attributes_of_query(self, attributes_of_query: List[Dict[str, Any]], additional_data_definitions: List[AdditionalDataDefinition]) -> List[Dict[str, Any]]:
        for attribute_query in attributes_of_query:
            if 'additional_data_definition_name_en' in attribute_query:
                additional_data_definition = more_itertools.first_true(additional_data_definitions,
                                          pred=lambda e: AnnofabApiFacade.get_additional_data_definition_name_en(e) == attribute_query['additional_data_definition_name_en'])
                ...
                if additional_data_definition is not None:
                    attribute_query['additional_data_definition_id'] = additional_data_definition['additional_data_definition_id']

            additional_data_definition= more_itertools.first_true(additional_data_definitions,
                                      pred=lambda e: e['additional_data_definition_name_id'] == attribute_query['additional_data_definition_name_id'])
            if additional_data_definition is None:
                logger.warning(f"additional_data_definition_name_id = {attribute_query['additional_data_definition_name_id']} の属性は存在しませんでした。")
                continue
    ...
    def _modify_annotation_query(self, project_id: str, annotation_query: Dict[str, Any]) -> Dict[str, Any]:
        ...
        * ``choice_name_en`` から ``choice`` に変換する。
        ...
        if 'label_name_en' in annotation_query:
            ...
            label = more_itertools.first_true(specs_labels, pred=lambda e: AnnofabApiFacade.get_label_name_en(e) == label_name_en)
        ...
        if annotation_query.keys() >= {'label_id', 'attributes'}:
            ...
            label = more_itertools.first_true(specs_labels, pred=lambda e: e['label_id'] == annotation_query['label_id'])
            if label is not None:
                self._modify_attribute_of_query(annotation_query['attributes'], label['additional_data_definitions'])
            else:
...
def parse_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        '--group_by', type=str, choices=[GroupBy.TASK_ID.value, GroupBy.INPUT_DATA_ID.value],
