...
class CopyAnnotation(AbstractCommandLineInterface):
    ...
    class CopyTasksInfo:
        ...
        def append(self, project_id: str, input_data: str, validate_type=None):
            ...
            if validate_type == self.INPUT_TYPECHECK_ENUM.TASK_AND_FILE:
                ...
                self.__from_task_frame_keys = [
                    TaskFrameKey(project_id=project_id, task_id=from_task, input_data_id=from_input)
                ]
                self.__to_task_frame_keys = [
                    TaskFrameKey(project_id=project_id, task_id=to_task, input_data_id=to_input)
                ]
                ...
                if from_task_or_none == None:
                    ...
                    for from_input, to_input in zip(from_input_id_list, to_input_id_list):
                        self.__from_task_frame_keys.append(
                            TaskFrameKey(project_id=project_id, task_id=from_task, input_data_id=from_input)
                        )
                        self.__to_task_frame_keys.append(
                            TaskFrameKey(project_id=project_id, task_id=to_task, input_data_id=to_input)
                        )
    ...
    def main(self):
        ...
        deleteAnnotation = delete_annotation.DeleteAnnotation()
