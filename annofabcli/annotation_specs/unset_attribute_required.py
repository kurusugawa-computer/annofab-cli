...
class UnsetAttributeRequired(CommandLine):
    """
    属性の必須制約を解除するコマンド。
    """
...
def parse_args(parser: argparse.ArgumentParser) -> None:
    ...
    attribute_group.add_argument(
        ...
        type=str,
        nargs="+",
        help="必須制約を解除する対象属性の属性ID。1個だけ指定して ``file://`` を先頭に付けると、属性IDを1行ずつ記載したファイルを指定できます。",
    )
    attribute_group.add_argument(
        "--attribute_name_en",
        type=str,
        nargs="+",
        help="必須制約を解除する対象属性の英語名。1個だけ指定して ``file://`` を先頭に付けると、属性名(英語)を1行ずつ記載したファイルを指定できます。",
    )
...
def main(args: argparse.Namespace) -> None:
    ...
    UnsetAttributeRequired(service, facade, args).main()
