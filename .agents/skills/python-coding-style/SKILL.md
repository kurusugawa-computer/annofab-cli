...
# model
* `dataclasses_json`よりは`pydantic`を使用する。理由は、`pydantic`の方が型ヒントのサポートが充実しているため。